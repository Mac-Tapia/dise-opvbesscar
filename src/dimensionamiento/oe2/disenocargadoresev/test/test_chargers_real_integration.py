...
- ✅ 38 socket columns detected [v5.2]
- ✅ 8,760 hourly timesteps confirmed
- ✅ 38 charger_simulation_*.csv files created [v5.2]
...
def test_real_charger_dataset():
    ...
    try:
        ...
        if df.shape[1] != 38:
            logger.error(f"❌ Expected 38 columns (v5.2), got {df.shape[1]}")
            return False
...
def test_csv_generation():
    ...
    if len(found_csvs) != 38:
        logger.warning(f"⚠️  Expected 38 files (v5.2), got {len(found_csvs)}")
        return False
