...
- Verify dimensions (8760 × 38) [v5.2]
- Test dataset builder with real charger data
- Verify 38 CSV files generated correctly
- Validate schema.json references

Expected Output:
- [OK] Real dataset loads successfully
- [OK] 38 socket columns detected [v5.2]
- [OK] 8,760 hourly timesteps confirmed
- [OK] 38 charger_simulation_*.csv files created [v5.2]
- [OK] schema.json updated with CSV references
"""
...
def test_real_charger_dataset():
    ...
    if not charger_path.exists():
        logger.error(f"[X] File not found: {charger_path}")
        return False

    try:
        df = pd.read_csv(charger_path)
        logger.info(f"[OK] File loaded: {charger_path}")
        logger.info(f"   Shape: {df.shape}")
        logger.info(f"   Rows: {df.shape[0]} (expected 8760)")
        logger.info(f"   Cols: {df.shape[1]} (expected 38) [v5.2]")

        # Validate dimensions
        if df.shape[0] != 8760:
            logger.error(f"[X] Expected 8760 rows, got {df.shape[0]}")
            return False

        if df.shape[1] != 38:
            logger.error(f"[X] Expected 38 columns (v5.2), got {df.shape[1]}")
            return False
        ...
        if min_val < 0 or max_val > 5.0:
            logger.warning(f"[!]  Value range unexpected: [{min_val:.3f}, {max_val:.3f}]")
...
def test_dataset_builder_with_real_chargers():
    ...
    try:
        # NOTE: Old builder moved to new structure
        from src.dataset_builder_citylearn import rebuild_oe2_datasets_complete as build_citylearn_dataset
        from pathlib import Path
        ...
        logger.info(f"[OK] Dataset built successfully")
        ...
        logger.error(f"[!]  Cannot import dataset builder (module not in path): {e}")
        logger.info("   -> This is OK for standalone test, proceed to file checks")
        return True  # Not a critical failure
    except Exception as e:
        logger.error(f"[X] Error building dataset: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_csv_generation():
    """Test 3: Verify 38 charger CSV files generated [v5.2]"""
    logger.info("")
    logger.info("=" * 80)
    logger.info("TEST 3: Verify 38 Charger CSV Files [v5.2]")
    logger.info("=" * 80)
    ...
    for out_dir in output_dirs:
        if out_dir.exists():
            ...
            if csvs:
                found_csvs = csvs
                logger.info(f"[OK] Found output directory: {out_dir}")
                break

    if not found_csvs:
        logger.warning(f"[!]  No charger CSV files found in tested directories")
        logger.info("   -> Dataset may not have been built yet")
        return None  # Not a failure, just not run

    logger.info(f"[OK] Found {len(found_csvs)} charger CSV files")

    if len(found_csvs) != 38:
        logger.warning(f"[!]  Expected 38 files (v5.2), got {len(found_csvs)}")
        return False

    # Sample a few CSVs [v5.2: 0, 19, 37]
    sample_csvs = [found_csvs[0], found_csvs[min(19, len(found_csvs)-1)], found_csvs[min(37, len(found_csvs)-1)]]
        ...
        try:
            ...
            for col in required_cols:
                if col not in df.columns:
                    logger.error(f"[X] Missing required column: {col}")
                    return False

        except Exception as e:
            logger.error(f"[X] Error reading {csv_path}: {e}")
            return False
...
def test_schema_integration():
    ...
    if not found_schema:
        logger.warning(f"[!]  No schema.json found in tested directories")
        return None  # Not a failure, just not run

    logger.info(f"[OK] Found schema.json: {found_schema}")
        ...
        for bname, building in buildings.items():
            ...
            if chargers:
                ...
                if csv_ref and csv_ref.startswith("charger_simulation_") and csv_ref.endswith(".csv"):
                    logger.info(f"      [OK] CSV reference format valid")
                else:
                    logger.warning(f"      [!]  CSV reference format unexpected: {csv_ref}")

        logger.info("[OK] TEST 4 PASSED")
        return True

    except Exception as e:
        logger.error(f"[X] Error reading schema: {e}")
        return False
