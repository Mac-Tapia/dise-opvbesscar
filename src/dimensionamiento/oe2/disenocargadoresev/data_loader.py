...
def load_bess_data(config: dict[str, Any]) -> BESSData:
    ...
    """
    try:
        capacity = float(config.get("bess_capacity_kwh", 940.0))   # v5.2: 940 kWh
        power = float(config.get("bess_power_kw", 342.0))          # v5.2: 342 kW
...
def load_chargers_data(json_path: Path | str) -> list[ChargerData]:
    ...
    Returns:
        Lista de 19+ ChargerData (38 total con 2 sockets por cargador) [v5.2]
...
def _create_default_chargers() -> list[ChargerData]:
    """Crea 19 cargadores default v5.2 (38 sockets totales × 2 c/u).

    Distribución:
    - 15 para motos (30 sockets)
    - 4 para mototaxis (8 sockets)

    Returns:
        Lista de 19 ChargerData
    """
    chargers = []

    # 15 cargadores para motos (2 tomas c/u = 30 sockets)
    for i in range(15):
        chargers.append(ChargerData(
            charger_id=i,
            max_power_kw=7.4,  # Mode 3: 32A @ 230V
            vehicle_type="moto",
            sockets=2
        ))

    # 4 cargadores para mototaxis (2 tomas c/u = 8 sockets)
...
def validate_oe2_complete(
    ...
    - Chargers: Mínimo 19, máximo 38 acciones (v5.2)
    ...
    try:
        ...
        if total_sockets < 19 or total_sockets > 256:
            results["errors"].append(
                f"Total charger actions {total_sockets} outside valid range [19, 256] v5.2"
            )
    except OE2ValidationError as e:
