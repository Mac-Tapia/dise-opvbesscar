...
Verifies:
1. Real charger dataset loads correctly (8760 × 128)
...
def check_real_charger_dataset() -> bool:
    ...
    try:
        ...
        if df.shape != (8760, 128):
            logger.error(f"❌ Expected shape (8760, 128), got {df.shape}")
            return False
        logger.info(f"   ✅ Shape correct: (8760, 128)")
...
def check_socket_structure() -> bool:
    """Verify socket names and distribution (112 motos + 16 mototaxis)."""
    logger.info("\n" + "="*70)
    logger.info("CHECK 2: Socket Structure")
    logger.info("="*70)
    ...
    try:
        ...
        moto_cols = [c for c in columns if c.startswith("MOTO_") and "TAXI" not in c]
        taxi_cols = [c for c in columns if "MOTOTAXI" in c]

        logger.info(f"   ✅ MOTO columns: {len(moto_cols)}")
        logger.info(f"   ✅ MOTOTAXI columns: {len(taxi_cols)}")

        if len(moto_cols) != 112:
            logger.error(f"❌ Expected 112 MOTO columns, got {len(moto_cols)}")
            return False
        if len(taxi_cols) != 16:
            logger.error(f"❌ Expected 16 MOTOTAXI columns, got {len(taxi_cols)}")
            return False

        logger.info(f"   ✅ Distribution correct: 112 motos + 16 mototaxis = 128 total")
...
def check_citylearn_requirements() -> bool:
    ...
    try:
        ...
        generated_cols_count = 128

        logger.info(f"   ✅ Per-socket input columns: {generated_cols_count}")
