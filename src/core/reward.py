...
class RewardWeights:
    ...
    Validated weight sets (from literature):
        CO2_DUAL_FOCUS (Iquitos isolated grid):
            w_direct=0.35, w_co2=0.30, w_ev=0.25, w_solar=0.05, w_grid=0.05
    ...
    """
    w_direct_co2: float = 0.35   # direct CO₂ reduction (EV electrification)
    w_co2: float = 0.30          # indirect CO₂ reduction (grid displacement by PV)
    w_ev: float = 0.25           # EV service satisfaction
    w_solar: float = 0.05        # PV self-consumption
    w_grid: float = 0.05         # grid stability / ramp penalty
    ...
    def co2_dual_focus(cls) -> "RewardWeights":
        """Optimal for isolated thermal grids (Iquitos validated)."""
        return cls(w_direct_co2=0.35, w_co2=0.30, w_ev=0.25, w_solar=0.05, w_grid=0.05)
