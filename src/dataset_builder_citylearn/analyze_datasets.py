#!/usr/bin/env python3
"""
Análisis consolidado de datasets enriquecidos para CityLearn v2.

Proporciona estadísticas completas de los 3 datasets (Solar, Chargers, BESS)
...
def analyze_solar_dataset(solar_path: str | Path = "data/oe2/Generacionsolar/pv_generation_citylearn_enhanced_v2.csv"):
    ...
    print("\n" + "="*110)
    print("📊 ANÁLISIS DATASET SOLAR ENRIQUECIDO")
    print("="*110)
    ...
    print(f"\n✅ Dataset: {solar_path.name}")
    print(f"   Filas: {len(df):,} | Columnas: {len(df.columns)} | 5 nuevas")
    
    print(f"\n📈 ESTADÍSTICAS 5 COLUMNAS NUEVAS:")
    for col in cols_nuevas:
        if col in df.columns:
            ...
            maximo = df[col].max()
            print(f"   {col}:")
            print(f"      Total: {total:>15,.0f} | Prom: {prom:>10.2f} | Máx: {maximo:>10.1f}")
...
def analyze_chargers_dataset(chargers_path: str | Path = "data/oe2/chargers/chargers_ev_ano_2024_enriched_v2.csv"):
    ...
    print("\n" + "="*110)
    print("📊 ANÁLISIS DATASET CHARGERS ENRIQUECIDO")
    print("="*110)
...
def analyze_all_datasets():
    """Ejecuta análisis de todos los datasets enriquecidos."""
    
    print("\n" + "="*110)
    print("🔗 ANÁLISIS CONSOLIDADO - DATASETS ENRIQUECIDOS PARA CityLearn v2")
    print("="*110)
    
    try:
        analyze_solar_dataset()
    except FileNotFoundError:
        print("⚠️  Dataset solar no encontrado")
    
    try:
        analyze_chargers_dataset()
    except FileNotFoundError:
        print("⚠️  Dataset chargers no encontrado")
