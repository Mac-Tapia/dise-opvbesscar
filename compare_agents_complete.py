...
class ComparativeAnalyzer:
        ...
        for agent in AGENTS:
            try:
                ...
                if results_path.exists():
                    ...
                    logger.info(f"  ✓ {agent} result JSON loaded")
                else:
                    logger.warning(f"  ✗ {agent} result JSON not found")

                # Load trace CSV
                if trace_path.exists():
                    self.data[agent]["trace"] = pd.read_csv(trace_path)
                    logger.info(f"  ✓ {agent} trace CSV loaded ({len(self.data[agent]['trace'])} rows)")
                else:
                    logger.warning(f"  ✗ {agent} trace CSV not found")

                # Load timeseries CSV
                if timeseries_path.exists():
                    self.data[agent]["timeseries"] = pd.read_csv(timeseries_path)
                    logger.info(f"  ✓ {agent} timeseries CSV loaded ({len(self.data[agent]['timeseries'])} rows)")
                else:
                    logger.warning(f"  ✗ {agent} timeseries CSV not found")
            ...
            try:
                ...
                if trace is not None and len(trace) > 0:
                    ...
                    if "grid_import_kw" in trace.columns:
                        self.results[agent]["total_grid_import_kwh"] = float(trace["grid_import_kw"].sum() / 60)  # Convert kW·min to kWh
                ...
                logger.info(f"  ✓ {agent} metrics extracted ({len(self.results[agent])} fields)")

            except Exception as e:
                logger.error(f"  ✗ Error extracting {agent} metrics: {e}")
    ...
    def generate_reward_comparison_graph(self) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error generating reward comparison: {e}")
    ...
    def generate_co2_comparison_graph(self) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error generating CO2 comparison: {e}")
    ...
    def generate_grid_comparison_graph(self) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error generating grid comparison: {e}")
    ...
    def generate_solar_utilization_graph(self) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error generating solar utilization graph: {e}")
    ...
    def generate_ev_charging_graph(self) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error generating EV charging comparison: {e}")
    ...
    def generate_oe2_4_6_4_evaluation(self) -> dict[str, Any]:
        ...
        logger.info("\n" + "="*70)
        logger.info(f"✨ SELECTED AGENT (OE2 4.6.4): {best_agent} ({normalized_scores[best_agent]:.1f}%)")
        logger.info("="*70)
    ...
    def save_oe2_report(self, oe2_result: dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.error(f"    ✗ Error saving OE2 report: {e}")
