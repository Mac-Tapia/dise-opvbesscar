...
- Generacion de dataset BESS para CityLearn (electrical_storage_simulation.csv)
- Graficas de dimensionamiento y SOC
- NUEVO: Simulacion con ARBITRAJE HP/HFP tarifas OSINERGMIN

NOTA: El balance energetico completo del sistema esta en:
      src/dimensionamiento/oe2/balance_energetico/balance.py

Valores v5.3 (2026-02-XX):
- 38 sockets (19 cargadores ├ù 2) @ 7.4 kW = 281.2 kW instalado
...
def load_mall_demand_real(
    ...
    - Rango: 01/01/2024 00:00 a 31/12/2024 23:00 (8,760 horas = 365 dias ├ù 24)
...
def load_ev_demand(ev_profile_path: Path, year: int = 2024) -> pd.DataFrame:
    ...
    - Formato v5.2: 38 columnas socket_XXX_charging_power_kw (estocastico)
      -> Suma todas las potencias de sockets (19 cargadores ├ù 2 tomas = 38 sockets)
...
def calculate_bess_capacity(
    ...
    - Capacidad = Deficit EV / (DOD ├ù Eficiencia)
...
def calculate_max_discharge_to_mall(
    ...
    - 18h: SOC=100%, cierre=22h -> horas_restantes=4h
      descarga_requerida = (100-20)% ├ù 1700 = 1,360 kWh
      ritmo = 1,360 / 4 = 340 kWh/h
      
    - 19h: SOC=80%, cierre=22h -> horas_restantes=3h
      descarga_requerida = (80-20)% ├ù 1700 = 1,020 kWh
...
def simulate_bess_solar_priority(
    ...
    =========================================
    CARGA:   input_kw ├ù dt -> almacenamiento_kwh (con perdida charging)
    DESCARGA: almacenamiento_kwh -> output_kw ├ù dt (con perdida discharging)
    
    Formulas:
    - energy_into_bess = power_charge_kw ├ù (eff_charge)
    - energy_from_bess = energy_stored ├ù (eff_discharge)
    - Balance: sum(energy_into_bess ├ù sqrt(eff)) Ôëê sum(energy_from_bess) / sqrt(eff)
    ...
    # ===========================================================================
    # NUEVAS METRICAS v5.4: Ahorros economicos e impacto COÔéé (por hora)
    # ===========================================================================
    ...
    for h in range(n_hours):
        ...
        if pv_remaining > 0.01 and current_soc < soc_max:
            ...
            # energy_to_store = power_charge_kw ├ù eff_charge
            ...
            if energy_to_store_kwh > 0.01:
                ...
                current_soc = min(current_soc, soc_max)  # Asegurar SOC Ôëñ 100%
        ...
        if (activar_descarga_ev or activar_descarga_picos):
            ...
            # PRIORIDAD DESCARGA:
            # 1┬║ BESS -> EV (cobertura 100% - maxima prioridad)
            # 2┬║ BESS -> MALL (limitar picos > 2000kW, si hay energia residual)
            # ---------------------------------------------------------------
        ...
        # =====================================================================
        # METRICAS v5.4: Ahorros y COÔéé evitado
        # =====================================================================
    ...
    df = pd.DataFrame({
        ...
        # ===================================================================
        # NUEVAS COLUMNAS v5.4: Ahorros e impacto COÔéé
        # ===================================================================
    ...
    # Reduccion COÔéé - DETALLADO CON BESS v5.4
    # =========================================================================
    # CO2 EVITADO = (PV directo + BESS discharge) ├ù factor CO2 generacion termica
    # 
    # La red publica Iquitos es generada por:
    # - Generacion termica: diesel B5 @ 0.4521 kg COÔéé/kWh (OSINERGMIN)
    # 
...
def simulate_bess_arbitrage_hp_hfp(
    ...
    - AHORRO: S/.0.17/kWh diferencial ├ù energia descargada en HP
    ...
    df = pd.DataFrame({
        ...
        'grid_export_kwh': pv_curtailed,  # Sin conexion a red
        'soc_percent': soc * 100,
        'soc_kwh': soc * capacity_kwh,
...
def run_bess_sizing(
    ...
    print("      - Playa Motos: 15 cargadores ├ù 2 = 30 tomas")
    print("      - Playa Mototaxis: 4 cargadores ├ù 2 = 8 tomas")
    ...
    print("   4. BESS descarga: Desde punto critico (PV<EV) hasta cierre 22h")
...
if __name__ == "__main__":
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  2´©ÅÔâú  TARIFAS OSINERGMIN MT3 - ELECTRO ORIENTE S.A. (IQUITOS)" + " "*17 + "|")
    print("+" + "-"*78 + "+")
    diferencial = TARIFA_ENERGIA_HP_SOLES - TARIFA_ENERGIA_HFP_SOLES
    print(f"""
    +---------------------------------------------------------------------+
    ...
    |                                                                     |
    |  ­ƒÆ░ DIFERENCIAL ARBITRAJE: S/.{diferencial:.2f}/kWh ({diferencial/TARIFA_ENERGIA_HFP_SOLES*100:.1f}% ahorro)          |
    |                                                                     |
    |  ­ƒîí´©Å FACTOR CO2 (Sistema termico aislado): {FACTOR_CO2_KG_KWH:.4f} kg/kWh         |
    +---------------------------------------------------------------------+
    """)
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  3´©ÅÔâú  DATOS DE ENTRADA - VERIFICACION DE ARCHIVOS" + " "*29 + "|")
    print("+" + "-"*78 + "+")
    ...
    print("\n    ­ƒôé ARCHIVOS DE ENTRADA:")
    for status in files_status:
        print(status)
    print(f"\n    ­ƒôü Directorio salida: {oe2_dir / 'bess'}")
    ...
    print("\n")
    print("+" + "-"*78 + "+")
    print("|  4´©ÅÔâú  EJECUTANDO DIMENSIONAMIENTO BESS..." + " "*37 + "|")
    print("+" + "-"*78 + "+")
    ...
    print("\n")
    print("+" + "-"*78 + "+")
    print("|  5´©ÅÔâú  RESULTADOS DIMENSIONAMIENTO BESS v5.3" + " "*35 + "|")
    print("+" + "-"*78 + "+")
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  6´©ÅÔâú  BALANCE ENERGETICO - FLUJOS DIARIOS Y ANUALES" + " "*27 + "|")
    print("+" + "-"*78 + "+")
    ...
    |  GENERACION                                                        |
    +---------------------------------------------------------------------+
    |  ÔÿÇ´©Å  PV Solar:         {result['pv_generation_kwh_day']:>10,.0f} kWh/dia  |  {pv_year:>8,.1f} MWh/ano     |
    +---------------------------------------------------------------------+
    |  DEMANDA                                                           |
    +---------------------------------------------------------------------+
    |  ­ƒÅó Mall:              {result['mall_demand_kwh_day']:>10,.0f} kWh/dia  |  {mall_year:>8,.1f} MWh/ano     |
    |  ­ƒöî EV Chargers:       {result['ev_demand_kwh_day']:>10,.0f} kWh/dia  |  {ev_year:>8,.1f} MWh/ano     |
    |  -----------------------------------------------------------------  |
    |  [GRAPH] TOTAL DEMANDA:     {result['total_demand_kwh_day']:>10,.0f} kWh/dia  |  {total_year:>8,.1f} MWh/ano     |
    +---------------------------------------------------------------------+
    |  BALANCE                                                           |
    +---------------------------------------------------------------------+
    |  Ô¼å´©Å  Excedente PV:      {result['surplus_kwh_day']:>10,.0f} kWh/dia  (PV - Demanda)        |
    |  Ô¼ç´©Å  Deficit EV:        {result['deficit_kwh_day']:>10,.0f} kWh/dia  (EV sin PV)           |
    +---------------------------------------------------------------------+
    |  INTERCAMBIO RED                                                   |
    +---------------------------------------------------------------------+
    |  ­ƒôÑ Import red:        {result['grid_import_kwh_day']:>10,.0f} kWh/dia  |  {grid_import_year:>8,.1f} MWh/ano     |
    |  ­ƒôñ Export red:        {result['grid_export_kwh_day']:>10,.0f} kWh/dia  |  {grid_export_year:>8,.1f} MWh/ano     |
    +---------------------------------------------------------------------+
    """)
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  7´©ÅÔâú  OPERACION BESS - SIMULACION ARBITRAJE HP/HFP" + " "*28 + "|")
    print("+" + "-"*78 + "+")
    ...
    |  METRICAS OPERACION                                                |
    +---------------------------------------------------------------------+
    |  ÔÜí Autosuficiencia:     {self_suff_value*100:>10.1f} %                           |
    |  ­ƒöä Ciclos/dia:          {result['cycles_per_day']:>10.2f}                             |
    |  [CHART] SOC minimo:          {result['soc_min_percent']:>10.0f} %                           |
    |  [CHART] SOC maximo:          {result['soc_max_percent']:>10.0f} %                           |
    |  Ôåö´©Å  Rango SOC:           {result['soc_min_percent']:.0f}% - {result['soc_max_percent']:.0f}%                            |
    +---------------------------------------------------------------------+
    """)
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  8´©ÅÔâú  METRICAS ECONOMICAS - AHORRO ARBITRAJE OSINERGMIN" + " "*23 + "|")
    print("+" + "-"*78 + "+")
    ...
    |  COSTOS ANUALES                                                    |
    +---------------------------------------------------------------------+
    |  ­ƒÆ© Costo BASELINE (sin BESS, sin PV):                             |
    |     S/.{cost_baseline:>15,.0f}/ano                                        |
    |                                                                     |
    |  ­ƒÆ░ Costo CON SISTEMA (PV + BESS + Arbitraje):                     |
    |     S/.{cost_with_bess:>15,.0f}/ano                                        |
    +---------------------------------------------------------------------+
    |  AHORROS                                                           |
    +---------------------------------------------------------------------+
    |  ­ƒÄ» Ahorro por arbitraje BESS:  S/.{savings_bess:>12,.0f}/ano                |
    |  ­ƒÄ» Ahorro TOTAL sistema:       S/.{savings_total:>12,.0f}/ano                |
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  9´©ÅÔâú  METRICAS CO2 - IMPACTO AMBIENTAL" + " "*40 + "|")
    print("+" + "-"*78 + "+")
    ...
    |  EMISIONES CO2                                                     |
    +---------------------------------------------------------------------+
    |  ­ƒÅ¡ BASELINE (sin PV, sin BESS):    {co2_baseline/1000:>10,.1f} ton CO2/ano      |
    |  ­ƒî▒ CON SISTEMA (PV + BESS):        {co2_emissions/1000:>10,.1f} ton CO2/ano      |
    +---------------------------------------------------------------------+
    |  REDUCCION INDIRECTA (BESS reemplaza energia termica de red)       |
    +---------------------------------------------------------------------+
    |  ­ƒî┐ REDUCCION INDIRECTA CO2:        {co2_avoided/1000:>10,.1f} ton CO2/ano      |
    ...
    # ===========================================================================
    print("+" + "-"*78 + "+")
    print("|  ­ƒöƒ ARCHIVOS GENERADOS" + " "*55 + "|")
    print("+" + "-"*78 + "+")
    ...
    print("\n")
    print("Ôòö" + "="*78 + "Ôòù")
    print("Ôòæ" + " "*25 + "[GRAPH] RESUMEN EJECUTIVO" + " "*33 + "Ôòæ")
    print("Ôòá" + "="*78 + "Ôòú")
    print(f"Ôòæ  ­ƒöï BESS:     {result['capacity_kwh']:,.0f} kWh / {result['nominal_power_kw']:,.0f} kW" + " "*(50-len(f"{result['capacity_kwh']:,.0f}")-len(f"{result['nominal_power_kw']:,.0f}")) + "Ôòæ")
    print(f"Ôòæ  ÔÿÇ´©Å  PV:       {pv_year:,.1f} MWh/ano generacion" + " "*(50-len(f"{pv_year:,.1f}")) + "Ôòæ")
    print(f"Ôòæ  ÔÜí Demanda:  {total_year:,.1f} MWh/ano (Mall + EV)" + " "*(49-len(f"{total_year:,.1f}")) + "Ôòæ")
    print(f"Ôòæ  ­ƒÆ░ Ahorro:   S/.{savings_total:,.0f}/ano ({reduction_pct:.1f}% reduccion)" + " "*(46-len(f"{savings_total:,.0f}")-len(f"{reduction_pct:.1f}")) + "Ôòæ")
    print(f"Ôòæ  ­ƒî┐ CO2:      {co2_avoided/1000:,.1f} ton reduccion indirecta/ano" + " "*(49-len(f"{co2_avoided/1000:,.1f}")) + "Ôòæ")
    print("Ôòá" + "="*78 + "Ôòú")
    print("Ôòæ  [OK] DIMENSIONAMIENTO BESS v5.3 COMPLETADO EXITOSAMENTE" + " "*24 + "Ôòæ")
    print("ÔòÜ" + "="*78 + "ÔòØ")
    print("\n")
