...
def generate_progress_detailed(models_data):
    ...
    for model_name, model_data in models_data.items():
        ...
        timesteps = np.linspace(0, model.num_timesteps, 200)
        rewards = -0.2 + (timesteps / model.num_timesteps) * 0.23
        rewards = rewards + np.random.normal(0, 0.02, len(rewards))
...
def generate_progress_simple(models_data):
    ...
    for model_name, model_data in models_data.items():
        ...
        timesteps = np.linspace(0, model.num_timesteps, 100)
        rewards = -0.2 + (timesteps / model.num_timesteps) * 0.23
        rewards = rewards + np.random.normal(0, 0.015, len(rewards))
