#!/usr/bin/env python3
"""
Validación ejecutable del cálculo de métricas de CityLearn v2.

Este script:
1. Carga la configuración y contexto de Iquitos
...
5. Muestra el cálculo detallado
"""
...
try:
    ...
    print("Asegúrate de tener los paquetes instalados: pip install -r requirements.txt")
    sys.exit(1)
...
def validate_episode1_calculation() -> None:
    """Ejecutar cálculo simulado del Episode 1."""
    
    print_header("VALIDACIÓN DE CÁLCULO: EPISODE 1 CITYLEARN V2")
    
    # ====================================================================
    # PASO 1: Cargar configuración
    # ====================================================================
    print("PASO 1: Cargar configuración")
    print("-" * 100)
    
    try:
        with open("configs/default.yaml", "r", encoding="utf-8") as f:
            config = yaml.safe_load(f)
        print(f"  ✓ Configuración cargada: configs/default.yaml")
    ...
    print(f"  ✓ Contexto Iquitos iniciado")
    print(f"    - CO₂ factor: {context.co2_factor_kg_per_kwh} kg/kWh")
    ...
    print(f"    - CO₂ grid minimization:  {weights.co2:.2f}")
    print(f"    - Solar self-consumption: {weights.solar:.2f}")
    print(f"    - EV satisfaction (MÁXIMA PRIORIDAD): {weights.ev_satisfaction:.2f}")
    ...
    print(f"  CO₂ reduction: {ref_co2_reduction:.1f}%")
    print()
    ...
    # Valores típicos durante operación en Episode 1
    # Asumimos una hora "promedio" donde el agente está funcionando bien
    grid_import_kwh = 7.2  # Bajo - PV directo es suficiente
    grid_export_kwh = 1.8  # Exceso solar
    solar_generation_kwh = 8.5
    ev_charging_kwh = 48.0  # Casi toda la demanda de PV directo
    ev_soc_avg = 0.835  # 83.5% - bueno, pero no perfecto
    bess_soc = 0.72  # BESS descargando
    hour = 14  # 2PM - hora con buen sol
    ev_demand_kwh = 50.0  # Demanda constante
    
    print(f"Métricas de hora simulada (2PM, soleado):")
    ...
    print(f"    r_co2 = -1.0 × ({grid_import_kwh} / ({grid_import_kwh} + {solar_direct})) ")
    print(f"    r_co2 = -1.0 × {grid_import_kwh / (grid_import_kwh + solar_direct):.4f}")
    print(f"    r_co2 = {r_co2_calc:.4f}")
    print(f"  Referencia: {ref_components['r_co2']:.4f}")
    print(f"  Estado: {'[OK] CORRECTO' if abs(r_co2_calc - ref_components['r_co2']) < 0.1 else '[ERROR] DISCREPANCIA'}")
    
    print("\nB. r_solar (0.20): Maximizar Autoconsumo Solar")
    print("-" * 100)
    print(f"  Formula: r_solar = 2.0 × (autoconsumo/solar_total) - 1.0")
    ...
    print(f"    r_solar = 2.0 × {autoconsumo_ratio:.4f} - 1.0")
    ...
    soc_target = 0.90
    r_soc = min(ev_soc_avg / soc_target, 1.0)  # Clip a 1.0
    ...
    print(f"    r_ev = 0.6 × {r_soc:.4f} + 0.4 × {charge_satisfaction:.4f}")
    print(f"    r_ev = {r_ev_calc:.4f}")
    print(f"  Referencia: {ref_components['r_ev']:.4f}")
    print(f"  Estado: {'[OK] CORRECTO' if abs(r_ev_calc - ref_components['r_ev']) < 0.1 else '[ERROR] DISCREPANCIA'}")
    print(f"  NOTA: Este es el componente CRITICO (peso 0.30)")
    
    print("\nD. r_cost (0.10): Minimizar Tarifa de Electricidad")
    print("-" * 100)
    print(f"  Formula: r_cost = -1.0 × (cost_normalized)")
    tariff = context.electricity_tariff_usd_per_kwh
    cost_usd = grid_import_kwh * tariff
    r_cost_calc = -1.0 * (cost_usd / (cost_usd + 50.0))  # Normalización
    print(f"  Cálculo:")
    print(f"    tariff = ${tariff}/kWh")
    print(f"    cost = {grid_import_kwh} × {tariff} = ${cost_usd:.2f}")
    print(f"    r_cost = -1.0 × ({cost_usd:.2f} / {cost_usd + 50.0:.2f})")
    ...
    print("-" * 100)
    print(f"  Formula: r_grid = 1.0 - 2.0 × (ramp_normalized)")
    grid_import_prev = 8.0  # Hora anterior: 8 kWh
    ...
    print(f"    r_grid = 1.0 - 2.0 × {ramp_normalized:.4f}")
    ...
    # ====================================================================
    print_header("PASO 5: CÁLCULO DE REWARD TOTAL (WEIGHTED SUM)")
    
    print("Fórmula: reward_total = Σ(weight_i × component_i)\n")
    ...
    if diff < 0.01:
        status = "✓ EXACTO (< 1% error)"
    elif diff < 0.05:
        status = "✓ CORRECTO (< 5% error)"
    else:
        status = f"⚠️  DISCREPANCIA ({diff_pct:.1f}% error)"
    
    print(f"Validación: {status}")
    print()
    
    # ====================================================================
    # PASO 6: Ahora calcular con el código real (MultiObjectiveReward.compute)
    # ====================================================================
    print_header("PASO 6: VALIDACIÓN CON CÓDIGO REAL (MultiObjectiveReward.compute)")
    ...
    for key, value in components_real.items():
        if key.startswith("r_"):
            print(f"  {key}: {value:8.4f}")    print()    
    print(f"\nReward total (código real): {reward_total_real:.4f}")
    print(f"Reward total (cálculo manual): {total_calc:.4f}")
    print(f"Diferencia: {abs(reward_total_real - total_calc):.6f}")
    print()
    
    # ====================================================================
    # PASO 7: Comparación con referencia documentada
    # ====================================================================
    print_header("PASO 7: COMPARACIÓN CON DOCUMENTACIÓN (EPISODE 1)")
    ...
    for name in ["r_co2", "r_solar", "r_ev", "r_cost", "r_grid"]:
        ...
        print(f"{match} {name:10s}: referencia={ref_val:8.4f}, cálculo={calc_val:8.4f}, diff={diff:8.4f}")
    ...
    # ====================================================================
    print_header("RESUMEN FINAL: VALIDACIÓN DE IMPLEMENTACIÓN")
    ...
    print(f"  [OK] Calculo manual de componentes: {sum(v*w for v,w in zip(components_calc.values(), [weights.co2, weights.solar, weights.ev_satisfaction, weights.cost, weights.grid_stability])):.4f}")
