...
sac_result = output_dir / "result_SAC.json"
ppo_result = output_dir / "result_PPO.json"
a2c_result = output_dir / "result_A2C.json"
...
while True:
    try:
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Elapsed: {elapsed:.1f} min | Checking for result files...")
        ...
        for agent_name, agent_info in agents_status.items():
            ...
            if not agent_info["found"] and result_file.exists():
                try:
                    ...
                    print(f"\n✅ {agent_name} COMPLETE at {agent_info['time'].strftime('%H:%M:%S')}")
                ...
                print(f"   ✅ {agent_name} - Completed at {agent_info['time'].strftime('%H:%M:%S')}")
            else:
