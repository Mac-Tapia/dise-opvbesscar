#!/usr/bin/env python3
"""
Diagnóstico: Verificar si hay carga BESS anómala en madrugada (300-600 kWh/h)

El BESS NO debería cargarse en la madrugada (00:00-05:59):
- EV está cerrado (cierra a las 22h)
- No hay generación solar
- La única carga debería estar en 06:00-22:59
...
def check_bess_midnight_charge():
    """Verifica si hay carga errónea en madrugada."""
    print("\n" + "=" * 80)
    print("🔍 DIAGNÓSTICO: Carga BESS en Madrugada")
    print("=" * 80)
    ...
    for name, path_str in DATASETS.items():
        ...
        try:
            ...
            if not charge_cols and not grid_bess_cols:
                print(f"   ⚠️  No se encontraron columnas de carga BESS")
                print(f"   Columnas disponibles: {', '.join(df.columns[:10])}")
                continue
            
            # Extraer hora de índice o columna datetime
            ...
            print(f"   📅 Período: {len(df)} horas = {len(df)//24} días")
            print(f"   🌙 Horas madrugada analizadas: {len(midnight_df)} ({len(midnight_df)//24} días × 6h)")
            ...
            for col in charge_cols + grid_bess_cols:
                ...
                # Estadísticas madrugada
                midnight_max = midnight_data.max()
                midnight_mean = midnight_data.mean()
                midnight_nonzero = midnight_data[midnight_data > 0.1].count()
                
                print(f"\n   🔋 Columna: {col}")
                print(f"      Total año: max={total_max:,.1f} kWh, media={total_mean:.1f} kWh")
                print(f"      Madrugada: max={midnight_max:,.1f} kWh, media={midnight_mean:.2f} kWh")
                print(f"      ⚠️  Horas con carga > 0.1 kWh en madrugada: {midnight_nonzero}/{len(midnight_data)}")
                
                # ALERTA: Si hay carga significativa
                if midnight_max > 10:
                    print(f"      ❌ ERROR: Carga de {midnight_max:,.1f} kWh en madrugada (debería ser 0)")
                    ...
                    print(f"      ✅ OK: Madrugada correctamente inactiva")
    ...
    print("   Razones:")
    print("   1. EV está cerrado (cierra 22h)")
    print("   2. No hay generación solar (noche)")
    ...
    print("\n✅ Corrección: Agregar `if hour_of_day < 6: grid_to_bess[h] = 0` al final de cada función")
