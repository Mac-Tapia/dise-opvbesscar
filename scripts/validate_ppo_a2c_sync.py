...
# -*- coding: utf-8 -*-
"""
Validador de Sincronizacion PPO vs A2C
================================================================================
Verifica que PPO y A2C cargan EXACTAMENTE los mismos datasets y tienen 
la misma logica de energia, de reward y metricas.
...
def validate_solar_datasets():
    """Verifica que solar sea idenetico entre PPO y A2C."""
    print("\n" + "="*80)
    print("VALIDACION 1: SOLAR GENERATION")
    print("="*80)
    
    solar_path = Path('data/interim/oe2/solar/pv_generation_citylearn_v2.csv')
    if not solar_path.exists():
        print(f"[X] FALLA: Solar no existe en {solar_path}")
        return False
    ...
    if 'pv_generation_kwh' not in df_solar.columns:
        print(f"[X] FALLA: Solar CSV no tiene columna 'pv_generation_kwh'")
    ...
    if len(solar_data) != 8760:
        print(f"[X] FALLA: Solar tiene {len(solar_data)} horas, esperaba 8760")
        return False
    
    total_solar = float(np.sum(solar_data))
    print(f"[OK] PASS: Solar cargado correctamente")
    print(f"   Path: {solar_path}")
    print(f"   Filas: {len(solar_data)}")
    print(f"   Total: {total_solar:,.0f} kWh/ano")
...
def validate_chargers_datasets():
    """Verifica que chargers sean identicos entre PPO y A2C."""
    print("\n" + "="*80)
    print("VALIDACION 2: CHARGERS (38 sockets)")
    print("="*80)
    
    charger_path = Path('data/oe2/chargers/chargers_ev_ano_2024_v3.csv')
    if not charger_path.exists():
        print(f"[X] FALLA: Chargers no existe en {charger_path}")
        return False
    ...
    if len(power_cols) == 0:
        print(f"[X] FALLA: Chargers CSV no tiene columnas 'charger_power_kw'")
    ...
    if chargers_data.shape[0] != 8760:
        print(f"[X] FALLA: Chargers tiene {chargers_data.shape[0]} horas, esperaba 8760")
        return False
    
    n_sockets = chargers_data.shape[1]
    if n_sockets < 38:
        print(f"[!]  ADVERTENCIA: Chargers tiene {n_sockets} sockets, esperaba 38")
        # Tomar solo los primeros 38
        chargers_data = chargers_data[:, :38]
        n_sockets = 38
    elif n_sockets > 38:
        print(f"[!]  Reduciendo de {n_sockets} a 38 sockets (v5.2)")
        chargers_data = chargers_data[:, :38]
        n_sockets = 38
    
    total_demand = float(np.sum(chargers_data))
    print(f"[OK] PASS: Chargers cargado correctamente")
    print(f"   Path: {charger_path}")
    print(f"   Sockets: {n_sockets}")
    print(f"   Filas: {chargers_data.shape[0]}")
    print(f"   Demanda total: {total_demand:,.0f} kWh/ano")
...
def validate_mall_datasets():
    """Verifica que mall demand sea identico entre PPO y A2C."""
    print("\n" + "="*80)
    print("VALIDACION 3: MALL DEMAND")
    print("="*80)
    ...
    if mall_path is None:
        print(f"[X] FALLA: Mall demand no encontrado en ninguna ruta")
        return False
    ...
    print(f"[OK] PASS: Mall demand cargado correctamente")
    print(f"   Path: {mall_path}")
    print(f"   Columna: {col}")
    print(f"   Filas: {len(mall_data)}")
    print(f"   Demanda total: {total_mall:,.0f} kWh/ano")
...
def validate_bess_datasets():
    """Verifica que BESS data sea identico entre PPO y A2C."""
    print("\n" + "="*80)
    print("VALIDACION 4: BESS SOC DATA")
    print("="*80)
    ...
    if bess_path is None:
        print(f"[X] FALLA: BESS data no encontrado en ninguna ruta")
        return False
    ...
    if 'bess_soc_percent' in df_bess.columns:
        ...
        # Tomar primera columna numerica que tenga SOC
        soc_cols = [c for c in df_bess.columns if 'soc' in c.lower()]
        if soc_cols:
            col = soc_cols[0]
        else:
            print(f"[X] FALLA: BESS CSV no tiene columna SOC")
            return False
    
    bess_data = df_bess[col].values[:8760].astype(np.float32)
    
    # Normalizar a [0,1] si esta en [0,100]
    ...
    print(f"[OK] PASS: BESS SOC cargado correctamente")
...
def validate_energy_balance():
    ...
    print("\n" + "="*80)
    print("VALIDACION 5: BALANCE ENERGETICO (Solar vs Demanda)")
    print("="*80)
    ...
    print(f"[OK] BALANCE ENERGETICO:")
    print(f"   Generacion Solar:  {solar_total:>15,.0f} kWh/ano")
    print(f"   Demanda Chargers:  {chargers_total:>15,.0f} kWh/ano ({chargers_total/total_demand*100:.1f}% de demanda)")
    print(f"   Demanda Mall:      {mall_total:>15,.0f} kWh/ano ({mall_total/total_demand*100:.1f}% de demanda)")
    print(f"   ------------------------------------")
    print(f"   Demanda Total:     {total_demand:>15,.0f} kWh/ano")
    print(f"   ------------------------------------")
    print(f"   Solar Coverage:    {solar_total/total_demand*100:>14.1f}% de demanda")
    print(f"   Grid Import Need:  {max(0, total_demand - solar_total):>15,.0f} kWh/ano")
    
    if solar_total < total_demand * 0.3:
        print(f"[!]  ADVERTENCIA: Solar solo cubre {solar_total/total_demand*100:.1f}% de demanda (< 30%)")
    else:
        print(f"[OK] Solar adecuado para el tamano de instalacion OE2")
    
    return True

def main():
    print("\n" + "="*80)
    print("VALIDACION DE SINCRONIZACION: PPO vs A2C")
    print("="*80)
    ...
    if all_pass:
        ...
        print("\nPPO y A2C estan sincronizados con los MISMOS datasets OE2 reales.")
        print("Los resultados de entrenamiento seran COMPARABLES y REPRODUCIBLES.")
        return 0
    else:
