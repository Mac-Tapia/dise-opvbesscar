#!/usr/bin/env python
"""
A2C TRAINING - GPU OPTIMIZED VERSION
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("analyses/logs/a2c_gpu_fixed.log", encoding='utf-8')
    ]
)
...
def main():
    logger.info("="*100)
    logger.info("A2C TRAINING - GPU OPTIMIZED")
    ...
    # 4. CHECKPOINTS
    logger.info(f"\n[STEP 4] CHECKPOINTS")
    ckpt_dir = Path("analyses/oe3/training/checkpoints/a2c_gpu")
    ...
    model = A2C(
        ...
        env,
        learning_rate=5e-4,
        n_steps=128,           # A2C usa n_steps pequeño (actor-critic)
        gamma=0.99,
        gae_lambda=0.95,
        ent_coef=0.01,         # Entropy para exploracion
        vf_coef=0.5,
        max_grad_norm=0.5,
        device=device,
        verbose=1
    )
    logger.info(f"A2C created on {device}")
    ...
    logger.info(f"Total: 17,520 timesteps (GPU acelerado - 2 EPISODIOS COMPLETOS - ~45-60 min)")
    ...
    try:
        ...
        # Save final
        model.save(str(ckpt_dir / "a2c_final.zip"))
        logger.info(f"Final model saved")
        
    except KeyboardInterrupt:
        logger.warning(f"Interrupted")
        model.save(str(ckpt_dir / "a2c_interrupted.zip"))
