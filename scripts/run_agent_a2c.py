...
================================================================================
Ejecuta evaluación del agente A2C entrenado.

Este script:
1. Carga el checkpoint más reciente de A2C (si existe)
2. Ejecuta evaluación (1 año = 8760 horas)
3. Guarda métricas y timeseries

Uso:
    python -m scripts.run_agent_a2c
    python -m scripts.run_agent_a2c --train          # Entrenar desde cero
    python -m scripts.run_agent_a2c --resume         # Continuar entrenamiento
    python -m scripts.run_agent_a2c --eval-only      # Solo evaluación (default)

Checkpoints: checkpoints/a2c/a2c_final.zip
================================================================================
"""
...
def main():
    parser = argparse.ArgumentParser(description="Agente A2C - Evaluación/Entrenamiento")
    parser.add_argument(
        "--config",
        type=str,
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("  🤖 AGENTE A2C (Advantage Actor-Critic)")
    logger.info("=" * 80)
    logger.info("")
    ...
    out_dir = rp.outputs_dir / "agents" / "a2c"
    out_dir.mkdir(parents=True, exist_ok=True)

    checkpoint_dir = rp.checkpoints_dir / "a2c"
    ...
    checkpoint_path = checkpoint_dir / "a2c_final.zip"
    ...
    if args.train or (args.resume and has_checkpoint):
        ...
        if not has_checkpoint:
            ...
            logger.warning("     Para entrenar: python -m scripts.run_agent_a2c --train")
        else:
    ...
    logger.info("  Iniciando agente A2C...")
    logger.info("")
    ...
    result = simulate(
        ...
        include_solar=True,
        # A2C específico
        a2c_timesteps=timesteps if mode == "train" else 8760,  # 1 año para eval
        a2c_resume_checkpoints=resume,
        a2c_n_steps=256,
        a2c_learning_rate=3e-4,
        a2c_checkpoint_freq_steps=1000,
        # ✅ NO CALCULAR BASELINES - Solo entrenamiento A2C
    ...
    summary = {
        "agent": "a2c",
        "description": "Advantage Actor-Critic (on-policy, simple)",
    ...
    logger.info("")
    logger.info("=" * 80)
    logger.info("  ✅ AGENTE A2C COMPLETADO")
    logger.info("=" * 80)
    logger.info("")
