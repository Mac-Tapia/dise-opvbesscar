#!/usr/bin/env python
"""
PPO TRAINING - DEBUG VERSION CON METRICAS Y VALIDACION
"""
import sys
import os
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("analyses/logs/ppo_debug.log", encoding='utf-8')
    ]
)
...
# ============================================================================
# CONFIGURACION DEL SCHEMA - MALL IQUITOS CON PLAYAS
# ============================================================================
SCHEMA_PATH = "data/processed/citylearn/iquitos_ev_mall/schema_pv_bess.json"  # CORRECTO: MALL con playas
...
def main():
    logger.info("="*100)
    logger.info("PPO TRAINING - MALL IQUITOS CON PLAYAS DE MOTOTAXIS Y MOTOS")
    logger.info("="*100)
    ...
    logger.info(f"  EDIFICIO: Mall_Iquitos (1 edificio)")
    logger.info(f"  PLAYAS: 2 playas de estacionamiento con carga")
    logger.info(f"    - Playa 1 (Mototaxis): 16 tomas")
    logger.info(f"    - Playa 2 (Motos): 112 tomas")
    logger.info(f"  TOTAL: 128 tomas (cada una con estado base independiente)")
    logger.info(f"  ACCIONES: 130 dims (128 tomas + 2 otros dispositivos)")
    logger.info(f"  OBSERVACIONES: 926 features (estados de tomas + sistema)")
    logger.info(f"  VARIANTE: PV + BESS (Solar + Battery Storage)")
    ...
    logger.info("  - Clip range: 0.2")
    logger.info("  - Entropy coef: 0.005")
    ...
    try:
        ...
        logger.info("\n" + "="*100)
        logger.info("[RESULTADO] PPO TRAINING COMPLETADO EXITOSAMENTE")
        logger.info("="*100)
