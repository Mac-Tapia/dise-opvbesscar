...
# -*- coding: utf-8 -*-
"""
train_ppo_citylearn.py — PPO SB3 sobre CityLearn v2 (PyTorch)
=======================================================================
Entrenamiento del agente PPO usando stable-baselines3 PPO con
StableBaselines3Wrapper(CityLearnEnv) — entorno nativo CityLearn v2.

Agente: stable_baselines3.PPO (Proximal Policy Optimization, PyTorch)
...
Uso:
    ...
    python scripts/train/train_ppo_citylearn.py --timesteps 8760  # smoke test

Checkpoints: checkpoints/PPO_CityLearn/ppo_<steps>_steps.zip
=======================================================================
"""
...
CHECKPOINT_DIR: Path = _PROJECT_ROOT / "checkpoints" / "PPO_CityLearn"
LOG_DIR: Path = _PROJECT_ROOT / "logs" / "training" / "ppo_citylearn"
RESULTS_DIR: Path = _PROJECT_ROOT / "outputs" / "ppo_training"
TENSORBOARD_DIR: Path = _PROJECT_ROOT / "logs" / "tensorboard" / "ppo_citylearn"

# Hiperparámetros PPO optimizados v10.0 (extraidos de train_ppo.py)
# Refs: Engstrom et al 2020, Andrychowicz et al 2021, Schulman et al 2017
def _lr_schedule(remaining: float) -> float:
    """Schedule lineal: 1e-4 → 0 (Engstrom 2020: crítico para estabilidad)."""
    return 1e-4 * remaining
...
PPO_HYPERPARAMS: dict = {
    ...
    "n_steps": 4096,
    "batch_size": 256 if _DEVICE == "cuda" else 128,
    # n_epochs=10: crítico para convergencia (con 3 epochs aprende muy lento)
    "n_epochs": 10,
    # gamma=0.88: reducido para episodios de 8760 steps (evita retornos explosivos)
    # gamma=0.99 con 8760 steps genera returns imposibles (0.99^8760 ≈ 0)
    "gamma": 0.88,
    # gae_lambda=0.92: reduce varianza en horizontes largos (Schulman 2017 Fig.4)
    "gae_lambda": 0.92,
    "clip_range": 0.2,
    # ent_coef=0.02: mayor exploración para reward multi-objetivo 5 componentes
    "ent_coef": 0.02,
    "vf_coef": 0.5,
    "max_grad_norm": 0.5,
    # LR schedule lineal 1e-4→0 (evita KL drift alto con n_epochs=10)
    "learning_rate": _lr_schedule,
    # target_kl=0.02: detiene update si KL>0.02 (guardian activo con 10 epochs)
    "target_kl": 0.02,
    "policy_kwargs": {
        # Red más grande para entorno multi-objetivo (5 componentes de reward)
        "net_arch": dict(
            pi=[256, 256, 128],   # Actor: 3 capas
            vf=[512, 512, 256],   # Critic: 3 capas (value landscape complejo)
        ),
        "activation_fn": torch.nn.Tanh,   # Tanh mejor que ReLU para PPO (bounded)
        "ortho_init": True,                # Inicialización ortogonal (Engstrom 2020)
    },
    "verbose": 1,
    "device": _DEVICE,
}
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(LOG_DIR / f"ppo_citylearn_{_ts}.log", encoding="utf-8"),
    ],
)
...
class CO2MetricsCallback(BaseCallback):
    """Registra métricas de CO₂ y solar cada episodio completo."""

    def __init__(self, env_fn, verbose: int = 0) -> None:
    ...
    def _on_rollout_end(self) -> None:
        ...
        # Loguear a tensorboard vía SB3
        self.logger.record("train/episode_co2_reward", ep_reward)

        ep_num = self.num_timesteps // 8760
        log.info(
            "Rollout Ep~%d | steps=%d | ep_reward_sum=%.4f",
            ep_num, self.num_timesteps, ep_reward,
        )
...
class EVMetricsCallback(BaseCallback):
    """Callback que registra métricas de EV y CO₂ durante el entrenamiento PPO.
    ...
    def __init__(self, log_freq: int = 8760, verbose: int = 0) -> None:
        ...
        # ── Trace/timeseries step-level (sincronizado con train_ppo.py) ──
    ...
    def _on_step(self) -> bool:
        ...
        if (
            ...
            log.info(
                "[CO₂ %5.1f%%ep] Ep %d │ F0=%.0f │ F1=%.0f │ F2(PPO)=%.0f kg/año (★proyectado) "
                "│ vs F0: -%.1f%%",
        ...
        if self.n_calls % self.log_freq == 0 and self.n_calls > 0:
            ...
            def _dlt(cur: float, ep1: float) -> str:
                return f"{cur - ep1:+.0f}"
            log.info(
                "Ep %d │ BALANCE ENERGÉTICO ANUAL (kWh) — [CSV]=fijo │ [PPO]=decisión agente",
                ep_num,
            )
            log.info(
            ...
            log.info(
                "  [PPO] EV motos entregado=%9.0f kWh Δ=%+.0f kWh │ mototaxis entregado=%9.0f kWh Δ=%+.0f kWh │ debt_viols=%d",
            ...
            log.info(
                ...
                "Solo F2 varía con las decisiones del agente PPO."
            )
            log.info(
            ...
            log.info(
                "  F2 CTRL PPO     │ %10.0f kg/año │ vs F0: -%6.1f%%"
                " │ %s vs Ep1: %+.0f kg (%+.2f%%)",
    ...
    def on_training_end(self) -> None:
        """Genera todas las gráficas al finalizar el entrenamiento PPO."""
        self.output_dir.mkdir(parents=True, exist_ok=True)
        ...
        if self.trace_records:
            ...
            pd.DataFrame(self.trace_records).to_csv(_tp, mode="a", index=False, header=_hdr)
            self.trace_records = []
            log.info("[DATA] trace_ppo.csv: flush final")
        if self.timeseries_records:
            _tsp = self.output_dir / "timeseries_ppo.csv"
            _hdr = not _tsp.exists()
            pd.DataFrame(self.timeseries_records).to_csv(_tsp, mode="a", index=False, header=_hdr)
            self.timeseries_records = []
            log.info("[DATA] timeseries_ppo.csv: flush final")
        if len(self._episodes) > 1:
            log.info("[GRAPH] Generando gráficas de episodios OE3 PPO...")
            self._generate_ev_plots()
        if len(self._kpi_steps_history) > 1:
            log.info("[GRAPH] Generando gráficas KPI diarios PPO...")
            self._generate_kpi_graphs()
        log.info("[GRAPH] ✓ Gráficas PPO guardadas en: %s", self.output_dir)
    ...
    def _generate_ev_plots(self) -> None:
        ...
        fig.suptitle("PPO — CO₂ Reducción OE3 por Episodio", fontsize=14, fontweight="bold")
        ax = axes[0, 0]
        ...
        plt.tight_layout()
        p = self.output_dir / "ppo_co2_oe3_episodios.png"
        fig.savefig(str(p), dpi=120, bbox_inches="tight"); plt.close(fig)
        log.info("[GRAPH] Guardado: %s", p)

        # ── Dashboard 2: Balance energético ────────────────────────
        fig, axes = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle("PPO — Balance Energético por Episodio", fontsize=14, fontweight="bold")
        panels = [
        ...
        ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
        plt.tight_layout()
        p = self.output_dir / "ppo_balance_energetico_episodios.png"
        fig.savefig(str(p), dpi=120, bbox_inches="tight"); plt.close(fig)
        log.info("[GRAPH] Guardado: %s", p)

        # ── Dashboard 3: EV & operación ───────────────────────────
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        fig.suptitle("PPO — Métricas EV por Episodio", fontsize=14, fontweight="bold")
        ...
        plt.tight_layout()
        p = self.output_dir / "ppo_ev_metricas_episodios.png"
        fig.savefig(str(p), dpi=120, bbox_inches="tight"); plt.close(fig)
        log.info("[GRAPH] Guardado: %s", p)
        ...
        csv_p = self.output_dir / "ppo_episodios_history.csv"
    ...
    def _generate_kpi_graphs(self) -> None:
        ...
        ax.set_ylim(0, 1); ax.set_title("PPO KPI: 1 − Load Factor")
        ax.set_xlabel("Miles de pasos"); ax.legend(); ax.grid(True, alpha=0.3)
        _save(fig, "kpi_load_factor.png")

        # Dashboard 2×3
        fig, axes = plt.subplots(2, 3, figsize=(18, 10))
        fig.suptitle("PPO — Dashboard KPI CityLearn", fontsize=14, fontweight="bold")
        kpi_list = [
...
# ===================== PPO DIAGNOSTICS CALLBACK =====================

class PPODiagnosticsCallback(BaseCallback):
    """Registra métricas internas del optimizador PPO para gráficas diagnósticas."""
    ...
    def _on_rollout_end(self) -> None:
        ...
        try:
            ...
            clip = kv.get("train/clip_fraction", None)
            ...
            if kl is not None:
                ...
                self.clip_fraction_history.append(float(clip) if clip is not None else 0.0)
    ...
    def on_training_end(self) -> None:
        """Genera gráficas diagnósticas PPO."""
        ...
        def _save(fig: _mpl_fig.Figure, name: str) -> None:
            ...
            fig.savefig(str(p), dpi=120, bbox_inches="tight")
            plt.close(fig)
            log.info("[PPO-DIAG] Guardado: %s", p)

        # KL divergence
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.plot(steps_k, self.kl_history, "blue", alpha=0.4, label="KL raw")
        ax.plot(steps_k, sm(self.kl_history), "blue", linewidth=2, label="Suavizado")
        ax.axhline(y=0.02, color="orange", linestyle="--", alpha=0.7, label="Umbral 0.02")
        ax.axhline(y=0.05, color="red", linestyle="--", alpha=0.7, label="Limite 0.05")
        ax.set_title("PPO: KL Divergence"); ax.set_xlabel("Miles de pasos"); ax.set_ylabel("KL")
        ax.legend(); ax.grid(True, alpha=0.3)
        _save(fig, "ppo_kl_divergence.png")

        # Clip fraction
        fig, ax = plt.subplots(figsize=(10, 5))
        ax.plot(steps_k, [v * 100 for v in self.clip_fraction_history], "green", alpha=0.4, label="Clip% raw")
        ax.plot(steps_k, sm([v * 100 for v in self.clip_fraction_history]), "green", linewidth=2)
        ax.axhline(y=30, color="red", linestyle="--", alpha=0.7, label="30%")
        ax.axhline(y=20, color="orange", linestyle="--", alpha=0.7, label="20%")
        ax.set_title("PPO: Clip Fraction (%)"); ax.set_xlabel("Miles de pasos"); ax.set_ylabel("%")
        ax.legend(); ax.grid(True, alpha=0.3)
        _save(fig, "ppo_clip_fraction.png")
        ...
        ax.axhline(y=0.1, color="red", linestyle="--", alpha=0.7, label="Min 0.1")
        ax.axhspan(ymin=float("-inf"), ymax=0.1, alpha=0.1, color="red")
        ax.set_title("PPO: Entropy"); ax.set_xlabel("Miles de pasos"); ax.set_ylabel("Entropía")
        ax.legend(); ax.grid(True, alpha=0.3)
        _save(fig, "ppo_entropy.png")

        # Value metrics (2 panels)
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8), sharex=True)
        ax1.plot(steps_k, self.value_loss_history, "red", alpha=0.4)
        ax1.plot(steps_k, sm(self.value_loss_history), "red", linewidth=2)
        ax1.set_title("PPO: Value Loss"); ax1.set_ylabel("Loss"); ax1.grid(True, alpha=0.3)
        ax2.plot(steps_k, self.explained_var_history, "teal", alpha=0.4)
        ax2.plot(steps_k, sm(self.explained_var_history), "teal", linewidth=2)
        ax2.axhline(y=0.5, color="green", linestyle="--", alpha=0.7, label="Objetivo > 0.5")
        ax2.set_title("PPO: Explained Variance"); ax2.set_xlabel("Miles de pasos")
        ax2.set_ylabel("EV"); ax2.legend(); ax2.grid(True, alpha=0.3)
        plt.tight_layout()
        _save(fig, "ppo_value_metrics.png")

        # Dashboard 2×2
        fig, axes = plt.subplots(2, 2, figsize=(14, 10))
        fig.suptitle("PPO — Diagnósticos del Optimizador", fontsize=14, fontweight="bold")
        for ax, data, title, color in [
            (axes[0, 0], self.kl_history, "KL Divergence", "blue"),
            (axes[0, 1], [v * 100 for v in self.clip_fraction_history], "Clip Fraction (%)", "green"),
            (axes[1, 0], self.entropy_history, "Entropy", "purple"),
            (axes[1, 1], self.explained_var_history, "Explained Variance", "teal"),
        ]:
            ax.plot(steps_k, sm(data), color=color, linewidth=2)
            ax.set_title(title); ax.set_xlabel("Miles pasos"); ax.grid(True, alpha=0.3)
        plt.tight_layout()
        _save(fig, "ppo_dashboard.png")

        # CSV
        df = pd.DataFrame({
            "timestep": self.steps_history,
            "kl_divergence": self.kl_history,
        ...
        csv_p = self.output_dir / "ppo_diagnostics_history.csv"
        df.to_csv(str(csv_p), index=False)
        log.info("[PPO-DIAG] CSV: %s", csv_p)
...
class ConvergenceRewardsCallback(BaseCallback):
    ...
    def __init__(self, output_dir: Path, agent_name: str = "ppo",
...
def load_or_create_ppo(env) -> tuple[PPO, int]:
    """Carga el último checkpoint PPO (con VecNormalize) o crea modelo nuevo."""
    CHECKPOINT_DIR.mkdir(parents=True, exist_ok=True)
    zips = sorted(CHECKPOINT_DIR.glob("ppo_*_steps.zip"))
    ...
    if zips:
        ...
        try:
            ...
            log.info("Checkpoint PPO cargado: %s (%d steps)", latest, steps_done)
            return model, steps_done
        except Exception as e:
            log.warning("Error cargando %s: %s — creando nuevo", latest, e)

    log.info("Creando nuevo agente PPO (SB3, PyTorch)...")
    model = PPO(env=env, tensorboard_log=str(TENSORBOARD_DIR), **PPO_HYPERPARAMS)
    return model, 0


def train(total_timesteps: int = TOTAL_TIMESTEPS, rebuild_schema: bool = False) -> None:
    """Loop principal de entrenamiento PPO en CityLearn v2."""
    log.info("=" * 70)
    log.info("PPO SB3 + IquitosEVChargingWrapper — Iquitos PV-BESS-EV")
    log.info("PyTorch: %s | CUDA: %s | Device: %s", torch.__version__, torch.cuda.is_available(), PPO_HYPERPARAMS["device"])
    log.info("Total timesteps: {:,} ({} ep × 8760 h)".format(total_timesteps, total_timesteps // 8760))
    log.info("Accion: 3D [bess(-1→+1), ev_motos_frac(0→1), ev_mototaxis_frac(0→1)]")
    ...
    # VecNormalize: crítico para estabilidad PPO con episodios de 8760 steps
    # (Engstrom 2020: normalizar returns reduce varianza en episodios largos)
    ...
    if vecnorm_path.exists():
        ...
        env.training = True
        log.info("VecNormalize cargado desde checkpoint: %s", vecnorm_path)
    else:
        env = VecNormalize(
            vec_env,
            norm_obs=True,
    ...
    log.info("Validando configuración del agente PPO...")
    if not validate_agent_config("PPO", num_episodes=total_timesteps // 8760,
    ...
    # 3. Cargar o crear modelo PPO
    model, steps_done = load_or_create_ppo(env)
    ...
    metrics_cb = CO2MetricsCallback(env_fn=None)
    ev_metrics_cb = EVMetricsCallback(log_freq=8760, verbose=1)
    ev_metrics_cb.output_dir = RESULTS_DIR
    ppo_diag_cb = PPODiagnosticsCallback(output_dir=RESULTS_DIR, verbose=0)
    convergence_cb = ConvergenceRewardsCallback(
        output_dir=RESULTS_DIR, agent_name="ppo", ep_len=8760, verbose=0
    )
    ...
    model.learn(
        ...
        callback=CallbackList([checkpoint_cb, metrics_cb, ev_metrics_cb, ppo_diag_cb, convergence_cb]),
        reset_num_timesteps=False,
        tb_log_name=f"ppo_{_ts}",
    )
    ...
    log.info("=" * 70)
    log.info("VALIDACIÓN POST-ENTRENAMIENTO PPO — 10 episodios determinísticos")
    log.info("=" * 70)
    N_VAL = 10
    ...
    # 8. Guardar result_ppo.json (misma estructura que train_ppo.py)
    log.info("Guardando result_ppo.json...")
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    result_summary: dict[str, Any] = {
        ...
        "output_files": {
            ...
            "trace":       str(RESULTS_DIR / "trace_ppo.csv"),
            "timeseries":  str(RESULTS_DIR / "timeseries_ppo.csv"),
            "result":      str(RESULTS_DIR / "result_ppo.json"),
        },
    }
    result_path = RESULTS_DIR / "result_ppo.json"
    with open(result_path, "w", encoding="utf-8") as _f:
        json.dump(result_summary, _f, indent=2, ensure_ascii=False, cls=_NumpyEncoder)
    log.info("[OK] result_ppo.json → %s", result_path)

    # 9. Verificar archivos de salida
    _trace_path = RESULTS_DIR / "trace_ppo.csv"
    _ts_path    = RESULTS_DIR / "timeseries_ppo.csv"
    if _trace_path.exists():
        _rows = sum(1 for _ in open(_trace_path, encoding="utf-8")) - 1
        log.info("[OK] trace_ppo.csv: %d registros → %s", _rows, _trace_path)
    else:
        log.warning("[!] trace_ppo.csv no generado")
    if _ts_path.exists():
        _rows = sum(1 for _ in open(_ts_path, encoding="utf-8")) - 1
        log.info("[OK] timeseries_ppo.csv: %d registros → %s", _rows, _ts_path)
    else:
        log.warning("[!] timeseries_ppo.csv no generado")

    log.info("=" * 70)
    log.info("ENTRENAMIENTO PPO COMPLETADO en %.1f s", elapsed)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PPO CityLearn v2 - Iquitos PV-BESS-EV")
