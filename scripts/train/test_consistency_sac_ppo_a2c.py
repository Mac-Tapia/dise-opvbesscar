...
def load_real_datasets() -> Dict[str, pd.DataFrame]:
    ...
    if chargers_path.exists():
        ...
        print(f"✅ Chargers: {len(datasets['chargers'])} filas, " + f"{list(datasets['chargers'].columns)[:5]}...")
    else:
...
def validate_dataset_completeness(datasets: Dict[str, pd.DataFrame]) -> Dict:
    ...
    for name, df in datasets.items():
        if df.empty:
            validation[name] = {"status": "❌ VACÍO", "rows": 0, "expected": 8760, "complete": False}
        else:
            rows = len(df)
...
def validate_co2_columns(datasets: Dict[str, pd.DataFrame]) -> Dict:
    ...
    required_columns = {
        "chargers": ["co2_directo_anual_acumulado_kg"],
        "bess": ["co2_avoided_kg"],
        "solar": ["energia_kwh"],
    ...
    for dataset_name, req_cols in required_columns.items():
        ...
        if missing:
            validation[dataset_name] = {"status": "⚠️  COLUMNAS FALTANDO", "found": found, "missing": missing}
        else:
            validation[dataset_name] = {"status": "✅ OK", "found": found}
...
def calculate_dataset_totals(datasets: Dict[str, pd.DataFrame]) -> Dict:
    ...
    if "chargers" in datasets and not datasets["chargers"].empty:
        ...
        if "co2_directo_anual_acumulado_kg" in df.columns:
            totals["co2_directo_total_kg"] = float(df["co2_directo_anual_acumulado_kg"].iloc[-1])
    ...
    if "solar" in datasets and not datasets["solar"].empty:
        ...
        elif "energia_kwh" in df.columns:
            totals["co2_indirecto_solar_kg"] = float(df["energia_kwh"].sum() * 0.4521)
    ...
    if "bess" in datasets and not datasets["bess"].empty:
        ...
        elif "co2_avoided_kg" in df.columns:
            totals["co2_indirecto_bess_kg"] = float(df["co2_avoided_kg"].sum())

    # TOTAL CO2 EVITADO
    totals["co2_total_evitado_kg"] = (
        totals.get("co2_directo_total_kg", 0)
