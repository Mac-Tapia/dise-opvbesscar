...
def main():
    ...
    try:
        ...
        has_core_loaders = "load_solar_data" in content and "load_bess_data" in content
        has_mall_loader = "load_mall_demand_data" in content

        if has_core_loaders and has_mall_loader and "OE2" in content:
            logger.info("[OK] data_loader.py has OE2 data integration")
            tests_passed += 1
        elif has_core_loaders:
            logger.error("[X] data_loader.py missing Mall integration")
        else:
            logger.error("[X] data_loader.py missing BESS integration")
    except Exception as e:
