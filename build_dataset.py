...
try:
    ...
    if schema_path.exists():
        ...
        with open(schema_path, 'r') as f:
            ...
            logger.info(f"[OK] Schema structure verified: {len(buildings)} buildings")
            
            # Check first building
            if buildings:
                b = buildings[0]
                logger.info(f"    Building 0: {b.get('name', 'unnamed')}")
                logger.info(f"      - Electric Vehicle Charger: {len(b.get('electric_vehicle_chargers', []))} units")
                logger.info(f"      - Battery: {'Yes' if b.get('battery') else 'No'}")
                logger.info(f"      - PV: {'Yes' if b.get('solar_electric_power_generation') else 'No'}")
